...
Before we begin lets define a normalized vector $\mathbf{s} = \mathbf{n} \times \mathbf{t}$, where $\times$ is the vector cross product operator.
With the vectors $\left\lbrace \mathbf{t},\mathbf{n},\mathbf{s} \right\rbrace$ we can define a Cartesian coordinate system, whose centre will be a triangle ray-intersection point $\mathbf{p}$.
Having all vectors defined with respect to this coordinate system will allow to compute more easily the quantities that will be introduced below.
The transformation matrix that will transform from world coordinates to this system using column vectors with homogeneous coordinates is
